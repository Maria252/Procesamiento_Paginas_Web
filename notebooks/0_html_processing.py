...
# Intentar obtener la API key de variables de entorno primero, luego usar fallback
...
- catalogo_tuberias: Si es aplicable, información sobre tuberías (tipo Schedule 40/80, diámetros, longitudes)
- especificaciones_tecnicas: Especificaciones técnicas de productos (material, temperaturas de operación, normas UL/ASTM)
- precios_informacion: Información de precios visible o rangos de precio mencionados
...
def call_gemini(prompt, model_name="gemini-1.5-flash", cache=None):
...
def normalize_response(data):
    ...
    if not isinstance(data, dict):
        ...
        keys = [
            ...
            'catalogo_tuberias', 'especificaciones_tecnicas', 'precios_informacion'
        ]
    ...
    key_mapping = {
        ...
        'catalogo_tuberias': ['catalogo_tuberias', 'pipe_catalog'],
        'especificaciones_tecnicas': ['especificaciones_tecnicas', 'technical_specifications'],
        'precios_informacion': ['precios_informacion', 'pricing_information']
    }
...
async def process_single_company(idx, row, total, blocked_sites, gpt_cache):
    ...
    try:
        ...
        result = {
            ...
            'catalogo_tuberias': data.get('catalogo_tuberias', 'No Information'),
            'especificaciones_tecnicas': data.get('especificaciones_tecnicas', 'No Information'),
            'precios_informacion': data.get('precios_informacion', 'No Information'),
        }
